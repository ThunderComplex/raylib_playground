...
#if !( \
    (defined(__cplusplus) && __cplusplus >= 202002L) || (defined(__STDC__) && __STDC__ == 1 && defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L) || defined(_MSC_VER) || defined(__OBJC__))
...
bool Clay__ElementIsOffscreen(Clay_BoundingBox* boundingBox)
{
    ...
    return (boundingBox->x > (float)context->layoutDimensions.width) || (boundingBox->y > (float)context->layoutDimensions.height) || (boundingBox->x + boundingBox->width < 0) || (boundingBox->y + boundingBox->height < 0);
}
