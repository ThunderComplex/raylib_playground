...
void calculateImpulse(Ball* ball1, Ball* ball2, Vector2 collisionNormal, float penetration)
{
    float m1 = ball1->mass;
    float m2 = ball2->mass;

    Vector2 relativeVelocity = { ball1->vel.x - ball2->vel.x, ball1->vel.y - ball2->vel.y };

    float relativeVelocityAlongNormal = Vector2DotProduct(relativeVelocity, collisionNormal);
    float impulseScalar = (-(1 - ball1->elasticity) * relativeVelocityAlongNormal) / (m1 + m2);
    impulseScalar *= 0.05f;

    float maxImpulse = 5.0f;
    impulseScalar = Clamp(impulseScalar, -maxImpulse, maxImpulse);
    // printf("%f\n", Vector2Length(relativeVelocity));

    if (Vector2Length(relativeVelocity) < 200.f) {
        ball1->vel.x = 0.f;
        ball1->vel.y = 0.f;
        ball2->vel.x = 0.f;
        ball2->vel.y = 0.f;
    } else {
        ball1->vel.x += impulseScalar * collisionNormal.x;
        ball1->vel.y += impulseScalar * collisionNormal.y;
        ball2->vel.x -= impulseScalar * collisionNormal.x;
        ball2->vel.y -= impulseScalar * collisionNormal.y;
    }
    ...
    ball1->pos.x += collisionNormal.x * penetration * correctionFactor;
    ball1->pos.y += collisionNormal.y * penetration * correctionFactor;
    ball2->pos.x -= collisionNormal.x * penetration * correctionFactor;
    ball2->pos.y -= collisionNormal.y * penetration * correctionFactor;
}

void handleCollision(Ball* ball1, Ball* ball2)
{
    float deltaX = ball1->pos.x - ball2->pos.x;
    float deltaY = ball1->pos.y - ball2->pos.y;
    float distance = hypot(deltaX, deltaY) - ball1->radius - ball2->radius;

    if (distance <= 0.001) {
        Vector2 collisionNormal = { deltaX, deltaY };
        Vector2Normalize(collisionNormal);

        float penetration = -distance;
        calculateImpulse(ball1, ball2, collisionNormal, penetration);
    }
}
...
int main(void)
{
    ...
    Ball balls[MAX_BALLS] = { { .pos = { GetScreenWidth() / 2.0f, GetScreenHeight() / 2.0f },
        .vel = { 200, 200 },
        .ppos = { 0 },
        .radius = 40,
        .friction = 0.99f,
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_RIGHT) || (IsKeyDown(KEY_LEFT_CONTROL) && IsMouseButtonDown(MOUSE_BUTTON_RIGHT))) {
            if (ballCount < MAX_BALLS) {
                balls[ballCount++] = (Ball) {
                    ...
                    .grabbed = false,
                    .mass = Remap((float)GetRandomValue(1, 4), 1, 4, 1.f, 10.f)
                };
            }
        }
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_MIDDLE)) {
            for (int i = 0; i < ballCount; i++) {
                if (!balls[i].grabbed)
                    balls[i].vel = (Vector2) { (float)GetRandomValue(-2000, 2000), (float)GetRandomValue(-2000, 2000) };
            }
        }
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            if (!ball->grabbed) {
                ...
                for (int j = 0; j < ballCount; j++) {
                    ...
                    // float deltaX = ball->pos.x - otherBall->pos.x;
                    // float deltaY = ball->pos.y - otherBall->pos.y;
                    // float distance = hypot(deltaX, deltaY) - otherBall->radius;

                    // if (distance <= ball->radius) {
                    //     // otherBall->vel.x += ball->vel.x / 2.f;
                    //     // otherBall->vel.y += ball->vel.y / 2.f;
                    //     // ball->vel.x /= 2.f;
                    //     // ball->vel.y /= 2.f;

                    //     ball->pos.x += deltaX * 0.05f;
                    //     ball->pos.y += deltaY * 0.05f;
                    //     float velocityFactor = 2.5f;

                    //     if (Vector2Length(ball->vel) > Vector2Length(otherBall->vel)) {
                    //         ball->vel.x = -ball->vel.x / velocityFactor * ball->elasticity;
                    //         ball->vel.y = -ball->vel.y / velocityFactor * ball->elasticity;

                    //         otherBall->vel.x = (-otherBall->vel.x - (ball->vel.x * velocityFactor)) * otherBall->elasticity;
                    //         otherBall->vel.y = (-otherBall->vel.y - (ball->vel.y * velocityFactor)) * otherBall->elasticity;
                    //     } else {
                    //         ball->vel.x = -ball->vel.x * velocityFactor * ball->elasticity;
                    //         ball->vel.y = -ball->vel.y * velocityFactor * ball->elasticity;
